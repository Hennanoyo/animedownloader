...
GPU preparation also enables CUDA hardware decoding. Decoded CUDA frames stay on the GPU for the playable NVENC path; only the thumbnail branch downloads frames to system memory for CPU-side sampling and sprite filters. The CPU-only path remains unchanged. The shared preparation filter graph also caps its internal frame buffering and uses a small filter-thread limit so an imbalance between the full-rate playable branch and the sparse thumbnail branch cannot grow system-memory usage without bound.
