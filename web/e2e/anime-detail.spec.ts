...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/release-groups", async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: "[{\"id\":\"019a0000-0000-7000-8000-000000000020\",\"name\":\"ExampleSubs\",\"slug\":\"examplesubs\",\"enabled\":true,\"active_parser_profile_version\":1,\"draft_parser_profile_version\":2}]",
    });
  });
...
test("reviews release parser profile health and validates activation", async ({ page }) => {
  const groupId = "019a0000-0000-7000-8000-000000000020";
  const profileId = "019a0000-0000-7000-8000-000000000021";
  const sampleId = "019a0000-0000-7000-8000-000000000022";
  let profileStatus = "draft";

  await page.route(
    `**/api/release-groups/${groupId}/parser-profiles`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify([
          {
            ...
            release_group_name: "ExampleSubs",
            version: 2,
            status: profileStatus,
            created_at: "2026-09-25T00:00:00Z",
            activated_at:
              profileStatus === "active" ? "2026-09-26T00:00:00Z" : null,
            rules: [
              {
                id: "019a0000-0000-7000-8000-000000000023",
                field: "episode_number",
                pattern: "Episode-(?<episode_number>\\d+)",
                priority: 10,
                required: true,
                flags: "i",
                transform: "to_int",
  ...
  await page.route(
    `**/api/release-groups/${groupId}/parser-samples`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify([
          {
            ...
            release_group_id: groupId,
            source: "nyaa",
            title: "[ExampleSubs] Frieren Episode-08 [1080p][HEVC]",
            created_at: "2026-09-25T00:00:00Z",
          ...
          },
          {
            ...
            release_group_id: groupId,
            source: "nyaa",
            title: "[ExampleSubs] Frieren Episode-09 [1080p][HEVC]",
  ...
  await page.route(
    `**/api/release-parser-profiles/${profileId}/health`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          ...
          failure_rate: 0.3,
          drift_signal: true,
          drift_reason: "parser failure rate is 30% across 10 observations",
          recent_failures: [
            {
              id: "019a0000-0000-7000-8000-000000000025",
              release_group_id: groupId,
              profile_id: profileId,
              source: "nyaa",
              title: "[ExampleSubs] Frieren Ep08 [1080p][HEVC]",
              status: "unparsed",
              failed_required_fields: ["episode_number"],
              observed_at: "2026-09-26T00:05:00Z",
            },
          ],
        }),
      });
  ...
  await page.route(
    `**/api/release-parser-profiles/${profileId}/comparison`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  await page.route(
    `**/api/release-parser-profiles/${profileId}/validate`,
    async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
          ...
          results: [
            {
              ...
              parsed: {
                provider_source: "nyaa",
                source_id: "sample:1",
                original_title: "[ExampleSubs] Frieren Episode-08 [1080p][HEVC]",
                normalized_title: "[ExampleSubs] Frieren Episode-08 [1080p][HEVC]",
                release_group: "ExampleSubs",
  ...
  await page.route(
    `**/api/release-parser-profiles/${profileId}/activate`,
    async (route) => {
  ...
  await page.getByRole("button", { name: "Validate samples" }).click();
  await expect(
    page.getByText("Validation passed across 2 samples. Activation is available."),
  ).toBeVisible();

  const activate = page.getByRole("button", { name: "Activate version" });
  await expect(activate).toBeEnabled();
  await activate.click();
  await expect(page.getByRole("heading", { name: /v2 · active/ })).toBeVisible();
});
...
test("discovers parsed releases from the anime detail page", async ({ page }) => {
  ...
  const groupInput = discovery.getByRole("combobox", {
    name: "Group",
    exact: true,
  });
  await groupInput.fill("Exam");
  await expect(
    page.getByRole("option", { name: "ExampleSubs", exact: true }),
  ).toBeVisible();
  await page.getByRole("option", { name: "ExampleSubs", exact: true }).click();
  await expect(groupInput).toHaveValue("ExampleSubs");
